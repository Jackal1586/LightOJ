...
struct myVec {
    ...
    myVec add ( myVec b ) {
        myVec res; for(int i=0;i<d;i++) res.val[i] = val[i] + b.val[i];return res;
    }
    myVec sub ( myVec b ) {
        myVec res; for(int i=0;i<d;i++) res.val[i] = val[i] - b.val[i];return res;
    }
    myVec mul ( double t ) {
        myVec res; for(int i=0;i<d;i++)res.val[i] = val[i] * t;return res;
    }
    myVec div ( double t ) {
        myVec res; for(int i=0;i<d;i++) res.val[i] = val[i] / t;return res;
    }
    bool operator == ( myVec b ) {
        for(int i=0;i<d;i++) if ( fabs ( val[i] - b.val[i] ) > eps ) return false; return true;
    }
    myVec perp2D() {
        myVec res = (*this);
...
int main(){
    ...
    for(int z=1;z<=t;z++){
        ...
        for(int i=0;i<n;i++){
            for(int j=0,k=m-1;j<m;k=j,j++){
                myLine q;
                q=q.lineFromPoints(out[j],out[k]);
                ans=min(ans,q.segmentToPointDistance(in[i],x));
            }
        }
        for(int i=0;i<m;i++){
            for(int j=0,k=n-1;j<n;k=j,j++){
                myLine q;
                q=q.lineFromPoints(in[j],in[k]);
                ans=min(ans,q.segmentToPointDistance(out[i],x));
            }
        }
        printf("Case %d: %.10lf\n",z,ans*0.5);
    }
